...
type FilterOprionsProps = {
  _order: Order;
  _orderBy: keyof DataFilter;
  refOrderBy: (
    orderBy: keyof DataFilter,
    order: Order
  ) => void;
};
...
export const FilterOptions: React.FC<FilterOprionsProps> = ({
  ...
  const handleSetOrderBy = (event: SelectChangeEvent<keyof DataFilter> ) => setOrderBy((event as SelectChangeEvent<keyof DataFilter>).target.value as keyof DataFilter);
  ...
  return (
    <FormControl fullWidth sx={{display: 'inline-flex', flexDirection: 'row', alignItems: "center"}}>
      ...
      <Select
        ...
        onChange={(event) => {handleSetOrderBy(event); refOrderBy(event.target.value as keyof DataFilter, order)}}
        sx={{width: '90%'}}
      >
        <SListSubheader>
          <ToggleButtonGroup
